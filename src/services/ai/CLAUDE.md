...
> 简单任务 = Prompt 文字调整、参数微调（temperature/max_tokens）、单函数 bug 修复；其余进 Plan 模式

- 影响哪个 Prompt（对话 / 记忆提取 / 历史压缩 / 主动发言 / 视觉分析）？
...
### 对话核心
- `chatStream` 签名冻结：`(userText: string, onChunk: (delta: string) => void) => Promise<void>`
- 记忆注入格式保持不变：`【关于用户的记忆】\n- content`
- 历史消息必须经 `normalizeHistory()` 处理，确保 user/assistant 交替（DeepSeek 要求）

### Prompt 修改规范
- 修改任意 Prompt 前，在 Plan 中写出「改前」和「改后」对比
- `extractMemoriesAsync` 和 `compressHistoryAsync` 的 Prompt 必须保持同步
- 主动发言 Prompt 调整需说明对发言频率的预期影响
...
完成后更新 `INTERFACE.md`，集成层凭此决定 App.tsx 中的调用方式和参数传递。
