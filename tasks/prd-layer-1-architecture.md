...
## US-004 交付：应用运行时层边界与能力范围

### 定位

应用运行时层是服务生命周期的独立归属层。它负责全部运行时能力的初始化、清理、配置变更响应、回调向上派发和运行中能力启停判断——但不负责窗口编排、页面渲染或用户交互入口。主协调层（App.tsx）在初始化时启动运行时层，在卸载时委托运行时层清理，运行时能力通过回调向上通知主协调层触发表情、气泡等表现。

### 应用运行时层职责清单

以下 5 项职责（源自 US-001 职责总表）归属于应用运行时层：

| # | 职责 | 当前位置（App.tsx 行号） | 说明 |
|---|------|------------------------|------|
| R-08 | **服务启动与清理** | L391-L541（init useEffect） | 初始化数据库（getDb），依次启动 weather / reminder / timeCycle / scheduler / colorSampler / screenMonitor 六个运行时能力，注册清理函数（useEffect return） |
| R-09 | **运行时配置响应** | L437-L441（listen settings-changed） | 监听 `settings-changed` 事件后重置 AI 客户端缓存（resetClient）、重新读取提醒间隔配置 |
| R-10 | **事件桥接：待办与专注** | L443-L480（listen all-todos-complete / focus-*） | 监听 `all-todos-complete` 触发 proudly 表情；监听 `focus-start/pause/reset/tick/phase-change/mouse-enter/mouse-leave` 更新 focusClock 状态和触发气泡 |
| R-11 | **事件桥接：CC 工作感知** | L483-L511（listen cc-event） | 监听 Claude Code 的 `cc-event`（PermissionRequest / Stop / PreToolUse / PostToolUse），控制表情变化、气泡显示和 ccActive 状态 |
| R-17 | **空闲计时** | L28-L29（IDLE_MS 常量）、L693-L706（resetIdle useEffect） | 30 分钟无交互后切换为 sleepy 表情；任何交互重置计时器 |

> **归属依据**：以上 5 项职责在 US-001 盘点中均被标记为「应用运行时层（迁出）」，对应 US-002 禁入清单中的 D-02（服务生命周期本体）、D-03（长生命周期轮询与监听治理）和 D-04（AI 到表现的协调本体）。

### 运行时能力集合

应用运行时层在 Layer 1 中只对以下 6 个运行时能力做归位，不做业务改写：

| 能力标识 | 服务入口 | 启动方式 | 清理方式 | 职责描述 |
|---------|---------|---------|---------|---------|
| `weather` | `startWeatherSync(callback)` | init 时启动，返回 `setInterval` 句柄 | `clearInterval` | 周期性同步天气数据，通过回调通知天气状态变化（如 rainy 表情） |
| `reminder` | `startReminderService(todoCallback, getInterval)` | init 时启动，返回 stop 函数 | 调用 stop 函数 | 按配置间隔检查待办，通过回调通知待办提醒（worried 表情 + 音效 + 气泡） |
| `scheduler` | `startSchedulerService(taskCallback)` | init 时启动，返回 stop 函数 | 调用 stop 函数 | 60 秒轮询 scheduled_tasks 表，匹配当前时间的任务通过回调触发提醒（happy 表情 + 气泡） |
| `screenMonitor` | `startScreenMonitor(config)` / `stopScreenMonitor()` | init 时启动 | `stopScreenMonitor()` | 周期性截图并分析屏幕内容，受低干扰模式节制，主动发起 AI 话题 |
| `timeCycle` | `startTimeCycleService(periodCallback)` | init 时启动，返回 stop 函数 | 调用 stop 函数 | 按时间段切换表情和气泡问候（如早中晚问候语） |
| `colorSampler` | `startColorSampler()` / `stopColorSampler()` | init 时启动 | `stopColorSampler()` | 采样屏幕颜色用于 UI 主题适配 |

> **约束声明**：Layer 1 不新增运行时能力、不修改能力的业务逻辑、不改变现有启动/清理时序。如果后续需要新增或移除运行时能力，属于 Layer 2 或更高层的决策。

### 常驻服务与条件服务区分

应用运行时层将 6 个能力分为两类：

#### 常驻服务（Resident Services）

常驻服务从应用初始化后持续运行直到应用关闭，不受用户配置或运行时数据状态的影响。它们维护自己的内部调度周期，持续产出输出。

| 能力标识 | 内部调度 | 产出 | 启动条件 |
|---------|---------|------|---------|
| `weather` | 固定周期同步 | 天气状态 → 表情映射（rainy） | 无条件启动 |
| `timeCycle` | 固定周期检测 | 时间段 → 表情/问候语映射 | 无条件启动 |
| `colorSampler` | 持续采样 | 屏幕颜色 → UI 主题适配 | 无条件启动 |

**常驻服务特征**：
- 启动不依赖数据库中是否存在特定数据
- 运行不依赖用户是否配置了特定参数
- 清理仅在应用关闭（useEffect return）时触发

#### 条件服务（Conditional Services）

条件服务虽然也在初始化时启动，但其运行时行为受用户配置、运行时数据或业务状态的影响。它们只在特定条件满足时才产生可见输出。

| 能力标识 | 条件依赖 | 触发条件 | 受影响的配置 |
|---------|---------|---------|------------|
| `reminder` | 待办数据存在、提醒间隔配置 | 有未完成待办 + 冷却期已过 + 优先级排序 | `reminder_interval_min`（settings 表），settings-changed 事件触发重新读取 |
| `scheduler` | 定时任务数据存在 | scheduled_tasks 表中有匹配当前时间的任务 | 无外部配置，依赖数据库数据 |
| `screenMonitor` | 低干扰模式状态、用户输入状态 | 非 disturbMode + 非 isUserTyping + 3 分钟节流已过 + 哈希去重通过 | `getDisturbMode()` 和 `isUserTyping()` 作为运行时查询传入 |

**条件服务特征**：
- 启动时即注册，但产出受运行时数据或配置节制
- 配置变更（如 `settings-changed`）会动态调整行为参数
- 数据状态（如待办列表为空、定时任务表为空）会直接抑制输出

### 行为不变约束

应用运行时层遵循以下约束：

1. **不改变现有业务行为**：Layer 1 只定义归属和边界，不修改任何运行时能力的启动顺序、回调逻辑、轮询频率或触发条件。所有服务的当前行为在 Layer 1 实施前后必须完全一致。

2. **不扩展为新的业务事件总线**：运行时层的回调向上派发机制（如 `setExpression('happy')`、`showSpeech(text, duration)`）是对现有调用模式的归位，不是新设计的事件总线。运行时层只负责"何时触发"，不负责"如何表现"——表现编排（表情切换序列、气泡事件链）仍由上层或专门的表现协调逻辑负责。

3. **不新增配置管理能力**：运行时层不引入新的配置管理机制，只在现有 `settings-changed` 监听的基础上做归位。

### 对 Layer 2~5 的前置条件

应用运行时层的存在为后续层提供以下明确的前置条件：

| 后续层 | 应用运行时层提供的前置条件 |
|-------|----------------------|
| **Layer 2（事件与状态层）** | 运行时层封装了全部服务启动/清理/配置响应逻辑，Layer 2 可以安全地在服务回调上建立统一的状态管理模式，无需担心与 App.tsx 的直接服务调用冲突。常驻/条件分类为 Layer 2 的服务状态机设计提供了基础分类。`settings-changed` 监听归位后，Layer 2 可以在此基础上构建统一的配置响应管道。 |
| **Layer 3（AI 交互层）** | screenMonitor 和 cc-event 的回调派发归位到运行时层后，Layer 3 的 AI 输出协议只需关注 AI 内容生成和意图理解，不需要关心 screenMonitor 的启动参数、节流逻辑或 cc-event 的内部状态跟踪（ccPermissionPending 标志）。AI 表现序列（thinking → happy → default + speech:* 事件链）的触发源和时序编排分离后，Layer 3 可以独立设计 AI 输出协议。 |
| **Layer 4（产品体验层）** | reminder 和 scheduler 的触发回调归位后，Layer 4 在设计产品体验时可以直接复用这些能力的触发入口，不需要重新理解 App.tsx 中的服务注册和回调逻辑。空闲计时（30 分钟 sleepy）归位后，Layer 4 可以在此基础上扩展更多的空闲行为设计，而不会与 App.tsx 的其他逻辑耦合。 |
| **Layer 5（工程与质量层）** | 运行时层的独立边界使得服务生命周期可以被单独测试和回归验证，Layer 5 建立的自动化检查可以针对运行时层编写，无需穿透整个 App.tsx。常驻/条件分类使得测试可以按类别设计：常驻服务测试启动/清理的确定性，条件服务测试各种条件分支。 |

### 本步骤的边界

- **只做**：定义应用运行时层的职责清单、运行时能力集合、常驻/条件服务区分、行为不变约束，以及对 Layer 2~5 的前置条件说明。
- **不做**：不实施代码迁移、不定义 hook 名称/控制器 API/文件拆分方案、不改变现有代码行为。
- **不做**：不展开运行时层内部的具体实现设计（如服务注册表、统一生命周期接口、配置管道），这些属于 Layer 2 的范围。
