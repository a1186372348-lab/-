...
**Claude Code 不会自动读取子目录的 CLAUDE.md，任务开始前必须手动读取对应文件。**

根据你当前所在的工作目录，执行对应操作：

| 工作目录 | 执行命令 |
|---|---|
| `cloud-tauri` | 读取 `src-tauri/CLAUDE.md` |
| `cloud-data` | 读取 `src/services/data/CLAUDE.md` |
| `cloud-ai` | 读取 `src/services/ai/CLAUDE.md` |
| `cloud-ui` | 读取 `src/components/CLAUDE.md` |
| `zhushou`（主控区） | 专属职责已在 `.claude/CLAUDE.md` 中自动生效 |

**如果你不确定自己在哪个目录，执行 `pwd` 或 `git branch --show-current` 确认后再读取对应文件。**
